...
const Calculator = () => {
  ...
  return (
    <>
      ...
      {/* Mobile version - CLEAN COPY, NO EFFECTS */}
      <section className="block lg:hidden py-8">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold mb-3 text-white">Calculate Your Potential Savings</h2>
            <p className="text-sm text-gray-400">See how much you could save by automating manual processes with AI</p>
          </div>

          <div className="space-y-4">
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <h3 className="text-lg font-bold text-white mb-4">Your Current Situation</h3>
              
              <div className="space-y-4">
                <div>
                  <label className="block text-white mb-2 text-sm">Number of Employees</label>
                  <input type="range" min="1" max="100" value={employees} onChange={(e) => setEmployees(parseInt(e.target.value))} className="w-full h-2 bg-white/20 rounded-lg mb-2" style={{ background: `linear-gradient(to right, #10b981 0%, #10b981 ${employees}%, rgba(255,255,255,0.2) ${employees}%, rgba(255,255,255,0.2) 100%)` }} />
                  <input type="number" min="1" max="100" value={employees} onChange={(e) => setEmployees(Math.max(1, Math.min(100, parseInt(e.target.value) || 1)))} className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm" />
                  <div className="mt-1 text-xs text-gray-400">{employees} employees</div>
                </div>

                <div>
                  <label className="block text-white mb-2 text-sm">Average Annual Salary</label>
                  <input type="range" min="1000" max="100000" step="1000" value={salary} onChange={(e) => setSalary(parseInt(e.target.value))} className="w-full h-2 bg-white/20 rounded-lg mb-2" style={{ background: `linear-gradient(to right, #10b981 0%, #10b981 ${((salary - 1000) / 99000) * 100}%, rgba(255,255,255,0.2) ${((salary - 1000) / 99000) * 100}%, rgba(255,255,255,0.2) 100%)` }} />
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-white text-sm">$</span>
                    <input type="number" min="1000" max="100000" step="1000" value={salary} onChange={(e) => setSalary(Math.max(1000, Math.min(100000, parseInt(e.target.value) || 1000)))} className="w-full bg-white/10 border border-white/20 rounded-lg pl-7 pr-3 py-2 text-white text-sm" />
                  </div>
                  <div className="mt-1 text-xs text-gray-400">{formatCurrency(salary)} per year</div>
                </div>

                <div>
                  <label className="block text-white mb-2 text-sm">Manual Hours Per Week</label>
                  <input type="range" min="1" max="60" value={hoursPerWeek} onChange={(e) => setHoursPerWeek(parseInt(e.target.value))} className="w-full h-2 bg-white/20 rounded-lg mb-2" style={{ background: `linear-gradient(to right, #10b981 0%, #10b981 ${(hoursPerWeek / 60) * 100}%, rgba(255,255,255,0.2) ${(hoursPerWeek / 60) * 100}%, rgba(255,255,255,0.2) 100%)` }} />
                  <input type="number" min="1" max="60" value={hoursPerWeek} onChange={(e) => setHoursPerWeek(Math.max(1, Math.min(60, parseInt(e.target.value) || 1)))} className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm" />
                  <div className="mt-1 text-xs text-gray-400">{hoursPerWeek} hours/week</div>
                </div>
              </div>
            </div>

            <div className="bg-gradient-to-br from-red-500/10 to-orange-600/10 border border-red-500/30 rounded-xl p-4">
              <h3 className="text-base font-bold text-white mb-3">Annual Cost of Manual Processes</h3>
              <div className="text-3xl font-bold text-white mb-3">{formatCurrency(annualManualCost)}</div>
              <div className="grid grid-cols-2 gap-2 text-gray-300">
                <div>
                  <div className="text-xs text-gray-400">Monthly</div>
                  <div className="text-sm font-semibold">{formatCurrency(monthlyManualCost)}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400">Weekly</div>
                  <div className="text-sm font-semibold">{formatCurrency(weeklyManualCost)}</div>
                </div>
              </div>
            </div>

            <div className="bg-gradient-to-br from-green-500/10 to-emerald-600/10 border border-green-500/30 rounded-xl p-4">
              <h3 className="text-base font-bold text-white mb-2">Annual Savings with AI</h3>
              <p className="text-xs text-green-400 mb-3">Save up to 75% on manual processes</p>
              <div className="text-3xl font-bold text-green-400 mb-3">{formatCurrency(annualSavings)}</div>
              <div className="grid grid-cols-2 gap-2 text-gray-300">
                <div>
                  <div className="text-xs text-gray-400">Monthly Savings</div>
                  <div className="text-sm font-semibold text-green-400">{formatCurrency(monthlySavings)}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400">Weekly Savings</div>
                  <div className="text-sm font-semibold text-green-400">{formatCurrency(weeklySavings)}</div>
                </div>
              </div>
            </div>
          </div>
